...
def generate_data():
    ...
    oscillation_amplitude = 0.9  # Amplitude of the oscillation
    oscillation_frequency = 0.9  # Frequency of the oscillation

    def stock_price_movement(x, t):
        """Simulate change in stock price with random noise, drift, and oscillating volatility."""
    ...
    for i, t in enumerate(times):
        dx,sigma = stock_price_movement(x, t)
...
FEATURE_N=5
FEATURE_N=1
actor = Actor_model((2+len(columns_to_normalize))*BACKS+2, FEATURE_N).to(device)
critic = Critic_model((2+len(columns_to_normalize))*BACKS+2, FEATURE_N).to(device)
...
for _ in range(500):
    ...
    # df1=reset_index(df1)
    # df1
    stock_prices=generate_data()
    df1=pd.DataFrame(stock_prices)
    df1.columns=['Close']
    df1
